...
/** Chord inversion choice shown in chord and progression editors. */
 ...
 * - **mode** - Builds the selected church mode.
 */
type ScaleTypeValue = 'major' | 'minor' | 'harmonic-minor' | 'major-pentatonic' | 'minor-pentatonic' | 'major-blues' | 'minor-blues' | 'mode';

/**
 * Modal scale values supported by scale input.
 *
 * - **ionian** - Builds an Ionian mode scale.
 * - **dorian** - Builds a Dorian mode scale.
 * - **phrygian** - Builds a Phrygian mode scale.
 * - **lydian** - Builds a Lydian mode scale.
 * - **mixolydian** - Builds a Mixolydian mode scale.
 * - **aeolian** - Builds an Aeolian mode scale.
 * - **locrian** - Builds a Locrian mode scale.
 */
type ScaleModeValue = 'ionian' | 'dorian' | 'phrygian' | 'lydian' | 'mixolydian' | 'aeolian' | 'locrian';

/**
 * Edit panel shown by the music embed dialog.
 *
 * - **none** - Shows shared display settings without a builder panel.
 * - **chord** - Shows chord editing controls.
 * - **scale** - Shows scale editing controls.
 * - **progression** - Shows chord-progression editing controls.
 */
type MusicEmbedEditMode = 'none' | 'chord' | 'scale' | 'progression';
...
/** Shared subset of MUI TextField props exposed by BaseTextInput. */
