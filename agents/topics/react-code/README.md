...
[Feature Mechanics](../architecture/feature-mechanics.md).
For the architectural boundary between REMVC views and React presentation, use
[REMVC Architecture](../architecture/remvc-architecture.md#view-and-presentation).
...
- function components should be rare outliers
- use a function component only for very small, entirely stateless rendering,
  such as a one-line wrapper or tiny utility component with no lifecycle,
  no refs, no effects, no owned state, and no meaningful object model
- if a component needs local state, lifecycle behavior, refs, effects, multiple
  handlers, or several semantic regions, implement it as a class
...
For Music Notebook shared presentation, use `src/mn/components` when a
component is intentionally reusable across features. Feature-private UI should
stay under the owning feature until reuse is real.
