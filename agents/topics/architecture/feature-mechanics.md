...
The broader application architecture is documented separately in [REMVC Architecture](remvc-architecture.md).
...
- controller services may intentionally use a feature-prefixed controller filename when that mirrors a namespaced service
- this reflects the feature namespace in both the service name and the filename, for example `editor:controller` and `editorController.jsx`
...
- that file should define ambient/global types and must not use `export`
- service interface names should mirror the service purpose, for example `PlayerService`
- when defining the service interface, derive the methods from the `this.implement(...)` call in the service constructor
...
- some features may use a feature-prefixed controller filename to mirror a namespaced service
...
- global stylesheets should only contain styles intended to apply broadly across many components
...
- if CSS or image assets need to be copied into `dist`, follow the placement rules in [Build And Asset Flow](build-and-assets.md)
- if runtime asset path questions arise, use [Build And Asset Flow](build-and-assets.md) as the source of truth
...
The complementary build-side questions are covered in [Build And Asset Flow](build-and-assets.md):
